...
**Last updated:** 2026-05-10 (PLAN §11 item 11: `IAnalyticsBackfillService` + `--backfill-analytics` CLI on Analyser).
...
- **In progress (analytics):** **PLAN §11 items 1–11** done through backfill CLI. Next is **item 12** (performance smoke note) or **13** (Migrations README).

---

## 3. Recommended next step (small slice)

**Do next:** [PLAN.md §11](./PLAN.md) item **12** — performance smoke note for materialization throughput (DESIGN NFR-3), or item **13** — extend **`src/Migrations/README.md`** for analytics scripts.

**Why this order:** Backfill and metrics are in place; remaining checklist items are documentation and non-code verification.
...
**As of last update:** items **1–11** in source control; ensure DbUp has applied through `010` if you use analytics tables locally.

**CLI:** `dotnet run --project src/Analyser -- --backfill-analytics` (optional `--max-games N`). Exits after printing counts; does not start Kestrel.
