...
**Last updated:** 2026-05-10 (PLAN §11 items 2–5: migrations `007`/`008`, `PgnGameHeaderMapper`, and `Game` analytics columns on insert).
...
- **In progress (analytics):** **PLAN §11 items 1, 4–5** done: migration `006`, `Game` DTO analytics fields, `PgnGameHeaderMapper.ApplyFromTags` in `PersistenceService.InsertGames`, extended `InsertGame`. Next is **items 2–3** (migrations `007` / `008`) then **6+** (piece values, move deriver, materialization).

---

## 3. Recommended next step (small slice)

**Do next:** [PLAN.md §11](./PLAN.md) item **6** — implement **`IPieceValues`** + **`GamePositionSummaryFactory`** with tests, then item **7** (`GameMoveDeriver`) before repository write-paths/materialization.
...
**As of last update:** items **1–5** are in source control; apply DbUp on your environment to materialize `007/008` if not already.
