...
**Last updated:** 2026-05-10 (PLAN §11 item 6: `IPieceValues`, `GamePositionSummaryFactory`, `GamePositionSummary` DTO).
...
- **In progress (analytics):** **PLAN §11 items 1–6** done through position-summary factory; next is **item 7** (`GameMoveDeriver`), then repository inserts and **item 9** (materialization).

---

## 3. Recommended next step (small slice)

**Do next:** [PLAN.md §11](./PLAN.md) item **7** — implement **`GameMoveDeriver`** + tests, then **item 8** (repository `SqlStatements` / inserts for `GameMove` and `GamePositionSummary`).
...
**As of last update:** items **1–6** in source control; ensure DbUp has applied through `010` if you use analytics tables locally.
...
- **Rollups (in code):** **`ClassicalPieceValues`** (`IPieceValues`, profile `classical-1-3-3-5-9-0`) and **`GamePositionSummaryFactory`** build **`GamePositionSummary`** DTOs from **`BoardPosition`** bitboards (DB insert still TODO — item 8/9).
