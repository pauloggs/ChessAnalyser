...
**Last updated:** 2026-05-10 (PLAN §11 item 9: `IAnalyticsMaterializationService` + `PersistenceService` wiring after board insert).
...
- **In progress (analytics):** **PLAN §11 items 1–9** done through materialization after `InsertBoardPositions`. Next is **item 10** (`IMetricRegistry` + reference metrics).

---

## 3. Recommended next step (small slice)

**Do next:** [PLAN.md §11](./PLAN.md) item **10** — implement **`IMetricRegistry`** plus two reference metric executors and tests.

**Why this order:** Metrics consume materialized `GameMove` / `GamePositionSummary` rows (DESIGN §3.4).
...
**As of last update:** items **1–9** in source control; ensure DbUp has applied through `010` if you use analytics tables locally.
